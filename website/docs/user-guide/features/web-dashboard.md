...
The default `hermes-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs a pseudo-terminal helper (`ptyprocess` on POSIX, `pywinpty` on native Windows) to spawn the embedded TUI. Install both with:

```bash
pip install 'hermes-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows). `pip install hermes-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.
...
- PTY helper — `ptyprocess` on POSIX or `pywinpty` on native Windows, installed by the `pty` extra (`pip install 'hermes-agent[web,pty]'`, or `[all]` covers both)
